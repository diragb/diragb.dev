...
const ConfirmedPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get('/api/newsletter/confirm', { params: { email, token } })
  ...
  return (
    <div className='relative flex justify-center items-center flex-col gap-6 w-screen max-w-[720px] min-h-screen mx-auto py-24 px-6 text-zinc-950 bg-zinc-50'>
      <CustomHead title='Subscription Confirmed - Dirag Biswas' />
      <Navbar />

      {status === 'loading' && (
        <div className='flex flex-col items-center gap-3 text-center'>
          <div className='h-10 w-10 rounded-full border-[3px] border-zinc-200 border-t-teal-600 animate-spin' />
          <p className='text-lg font-inter tracking-[-0.025em] text-zinc-500'>Confirming your email...</p>
        </div>
      )}

      {status === 'success' && (
        <div className='flex flex-col items-center gap-4 text-center'>
          <div className='flex items-center justify-center h-16 w-16 rounded-full bg-teal-100 text-teal-700 text-3xl'>
            &#10003;
          </div>
          <h1 className='text-4xl sm:text-5xl leading-tight'>You&apos;re in!</h1>
          <p className='text-lg font-inter tracking-[-0.025em] text-zinc-600 max-w-sm'>
            Your email has been confirmed. You&apos;ll hear from me when something new goes live.
          </p>
          <Link
            href='/blog'
      ...
      {status === 'already_confirmed' && (
        <div className='flex flex-col items-center gap-4 text-center'>
          ...
          <h1 className='text-4xl sm:text-5xl leading-tight'>Already confirmed</h1>
          <p className='text-lg font-inter tracking-[-0.025em] text-zinc-600 max-w-sm'>
            This email is already on the list. No further action needed!
          </p>
          <Link
            href='/blog'
            className='mt-2 text-base font-inter tracking-[-0.075em] text-teal-600 transition-colors hover:text-teal-700 hover:underline hover:underline-offset-2 decoration-wavy'
